<template>
  <div class="about-us-warp">
    ...
    <div class="cont">
      <div class="cu-left">
        <div class="l-top"><span>关于我们</span><span class="fa fa-plus add" title="新增"  v-on:click="onclick('add')"></span></div>
        <ul class="l-cont">
      ...
      <div class="cu-right">
        ...
        <div class="r-word" v-if="!showContType">
          <commonKK :kkData="editData" :target="about_target" ref="commonKK" @kkTitle="getTitle" @getKKData="getData" @getKKBack="getBack"></commonKK>
        </div>
      </div>
    </div>
  </div>
  ...
  export default{
    ...
    data(){
      return {
        ...
        save:false,
        currentId:0,
        about_target:'about-us'
      }
    },
    methods: {
      async getListData(){
        let r = await axios.get('/about-us', {})
        if(r && r.code == 0){
          this.aboutUsData = r.data
      ...
      async onclick(type,id){
        switch(type){
          ...
          case 'edit':
            ...
            break
          case 'delete':
            var r = await axios.post('/about-us/delete',{id:id})
            if(r && r.code == 0){
          ...
          case 'save':
            ...
            // var r = await axios.post('/about-us/update',{id:3,title:'保帅',content:'4554544445444'})
            break
          case 'back':
          ...
          case 'add':
            ...
            break
          case 'show':
            var r = await axios.post('/about-us/update',{id:id,show:false})
            if(r && r.code == 0){
            ...
            break
          case 'hide':
            var r = await axios.post('/about-us/update',{id:id,show:true})
            if(r && r.code == 0){
      ...
      async getData(val){//获取组件common-kk返回值
        if(val.id){//id存在编辑更新
          let r = await axios.post('/about-us/update',{id:val.id,title:val.title,content:val.content})
          if(r && r.code == 0){
            Notification.success({
              title: '成功',
              message: '保存成功',
          ...
          let r = await axios.post('/about-us/create',{title:val.title,content:val.content})
          if(r && r.code == 0){
            Notification.success({
              title: '成功',
              message: '保存成功',
