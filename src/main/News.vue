  ...
  <div class="news-warp">
    <div class="p-warp">
      <div class="nav-search">
        <el-form size="small" :inline="true" :model="news" class="p-form-inline">
          <el-form-item label="新闻种类">
            <el-select clearable  v-model="news.type" @clear="clear('type')" placeholder="全部种类">
              <el-option  
                v-for="item in options"
                :key="item.blockNewsId"
                :label="item.blockNewsName"
                :value="item.blockNewsId">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="发布状态">
            <el-select clearable  v-model="news.status" @clear="clear('status')" placeholder="所有状态">
      ...
      <div class="nav-menu">
        <el-row>
          ...
          <el-button type="primary" size="small" @click="menuClick('addExp')">添加新闻</el-button>
    ...
    <el-dialog
      ...
      <el-form :model="formType" class="demo-ruleForm" :rules="rules" ref="ruleForm" >
        ...
        <el-form-item label="新闻描述:" label-width="100px">
          <el-input v-model="formType.blockNewsDes" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
      ...
      <el-dialog
        ...
        <template>
          ...
          <el-checkbox-group v-model="checkedBlocks" @change="handleCheckedBlock">
            <el-checkbox v-for="item in options" :label="item.blockNewsId" :key="item.blockNewsId">{{item.blockNewsName}}</el-checkbox>
          </el-checkbox-group>
        </template>
        <span slot="footer" class="dialog-footer">
        ...
        <el-dialog
          ...
          <div class="addExp-body">
            <div class="type">
              ...
              <el-button type="primary" @click="expSubmit">提交新闻</el-button>
            </div>
            <div class="exp-name">
              <span>新闻标题：</span>
  ...
  export default{
    ...
    data(){
      return {
        ...
        options:[],
        expDialogTitle:'添加新闻',//新闻dialog的标题名次
        expId:'',//新闻编辑对应ID
        expType:'',//新闻tan窗种类选中值
        expTitle:'',//新闻tan窗新闻标题
        expContent:'',//新闻tan窗新闻编辑框内容
        ExpDialogChange:false,//添加、编辑新闻dialog发生改变
        pubExpRadio: '1',//新闻是否发布：1代表发布；2代表暂不发布
        currentPage: 1,
        tableData: [],
    ...
    methods: {
      ...
      async getType(){
        let r = await axios.get('/news-block')
        if(r && r.code == 0){
          this.options = r.data
        }
      },
      async onSearch(){
        ...
        let params = {
          blockNewsId:_this.news.type,
          status:_this.news.status,
          page:currentPage,
          size:currentSize
        }
        let r = await axios.post('/news-single/search',
          {blockNewsId:params.blockNewsId,status:params.status,page:params.page,size:params.size})
        if(r && r.code == 0){
      ...
      handleEdit(index, row) {//表格中内容编辑
        ...
        this.expDialogTitle = '编辑新闻'
        this.pubExpRadio = row.status.toString()
        this.expId = row.singleNewsId
        this.expType = row.blockNewsId
      ...
      handleDelete(index, row) {//表格中内容删除
        let _this = this
        this.$confirm('确认删除该条新闻信息？')
          .then(async _ => {
            let r = await axios.post('/news-single/delete',{id:row.singleNewsId})
            if(r && r.code == 0){
      ...
      handleClose(done){//添加、编辑新闻的谈窗关闭按钮X
      ...
      handleExpTypeChanged(expType){//添加、编辑新闻的种类选中值发生改变
        this.ExpDialogChange = true
        // console.log(expType)
      },
      handleExpTitleChanged(expTitle){//添加、编辑新闻标题值发生改变
        this.ExpDialogChange = true
        // console.log(expTitle)
      },
      handleExpDialogClose(){//新闻dialog关闭的回调
      },
        ...
        switch(type){
          ...
          case 'batchDelete'://批量删除
            ...
            _this.multipleSelection.forEach(item =>{
              deleteData.push(item.singleNewsId)
            })
            this.$confirm(`确认删除选中${deleteData.length}条新闻信息？`)
              .then(async _ => {
                let r = await axios.post('/news-single/delete',{id:deleteData})
                if(r && r.code == 0){
                  Notification.success({
                    title: '成功',
                    message: '删除新闻成功',
        ...
        switch(type){
          ...
          case 'addType-save'://添加种类--弹窗保存
            ...
            _this.$refs['ruleForm'].validate(async (valid) => {
              if (valid) {
                 let r = await axios.post('/news-block/create',{blockNewsName:_this.formType.name,blockNewsDes:_this.formType.blockNewsDes})
                if(r && r.code == 0){
                  Notification.success({
                    title: '成功',
                    message: '添加新闻种类成功',
          ...
          case 'deleteType-cancel'://删除种类--弹窗取消
            ...
            this.checkedBlocks = []
            break
          case 'deleteType-save'://删除种类--弹窗保存/news-block/
            let  r = await axios.post('/news-block/delete',{id:this.checkedBlocks})
            if(r && r.code == 0){
              Notification.success({
                title: '成功',
                message: '删除'+r.data+'新闻种类成功',
                type:'success'
              })
              this.getType()
              this.dialog_deleteType = false
      ...
      async expSubmit(){//新闻提交
        ...
        let  r = await axios.post('/news-single/create',
          { id:params.id,blockNewsId:params.blockId,title:params.title,content:params.content,status:params.status})
        if(r && r.code == 0){
          let nfTitle = this.expId ? '编辑新闻信息成功':'创建新闻信息成功'
          Notification.success({
            title: '成功',
            message: nfTitle,
            type:'success'
          ...
          let nfTitle = this.expId ? '编辑新闻信息失败':'创建新闻信息失败'
          Notification.error({
            title: '失败',
            message: nfTitle,
            type:'error'
    ...
    //       this.blocks = val.map(item =>Object.assign({id: item.blockNewsId, name: item.name}))
    //     },
